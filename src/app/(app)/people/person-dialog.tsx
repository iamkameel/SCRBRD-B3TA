    ...
    if (open) {
      if (mode === 'edit' && person) {
        form.reset({
          ...
          dateOfBirth: person.dateOfBirth ? person.dateOfBirth : undefined,
        });
      } else {
        form.reset({
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <Tabs defaultValue="basic-info" className="space-y-4">
              ...
              <TabsContent value="basic-info" className="space-y-4 p-1">
                  ...
                  <FormField control={form.control} name="phone" render={({ field }) => (<FormItem><FormLabel>Phone (Optional)</FormLabel><FormControl><Input placeholder="+123456789" {...field} value={field.value ?? ''} disabled={isPending}/></FormControl><FormMessage /></FormItem>)} />

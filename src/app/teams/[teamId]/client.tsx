    ...
    startTransition(async () => {
        try {
            ...
            toast({
                ...
                description: `The person has been added to the team.`,
            });
            setOpen(false);
            form.reset();
            router.refresh();
            ...
            toast({
              title: "Error",
              description: error instanceof Error ? error.message : "Could not add player to roster.",
              variant: "destructive",
            });
        }
    });
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} value={field.value ?? ""} defaultValue="Player" disabled={isPending}>
             ...
             <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} value={field.value ?? ""} defaultValue="active" disabled={isPending}>
                ...
                initialRoster.map(member => (
                  <TableRow key={member.assignmentId}>
                    <TableCell className="font-medium flex items-center gap-2">
                      <Link href={`/players/${member.personId}`} className="hover:underline">
                        {member.personName}
                      </Link>
      ...
      <Card>
        ...
        <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-6">
                ...
                <StatItem label="Drawn" value={teamStats.matchesDrawn} />
                <StatItem label="Runs Scored" value={teamStats.totalRunsScored} />
                <StatItem label="Wickets Taken" value={teamStats.totalWicketsTaken} />
                <StatItem label="Net Run Rate" value={teamStats.netRunRate.toFixed(2)} />
            </div>
        </CardContent>
      </Card>
    </div>

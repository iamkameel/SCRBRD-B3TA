...
type TeamLevel = 'All' | 'A' | 'B' | 'C' | 'D/Other';

const getTeamLevel = (teamClass?: string): TeamLevel => {
    if (!teamClass) return 'D/Other';
    const upperClass = teamClass.toUpperCase();
    if (upperClass.includes('1ST') || upperClass.endsWith('A')) return 'A';
    if (upperClass.includes('2ND') || upperClass.endsWith('B')) return 'B';
    if (upperClass.includes('3RD') || upperClass.endsWith('C')) return 'C';
    return 'D/Other';
}
...
export default function TeamsClient({ teams, schools, divisions, seasons, canManage, coaches }: { teams: Team[]; schools: School[]; divisions: Division[]; seasons: Season[], canManage: boolean; coaches: Person[] }) {
  ...
  const [levelFilter, setLevelFilter] = React.useState<TeamLevel>('All');
  ...
  const filteredTeams = teams.filter(team => {
    ...
    const matchesLevel = levelFilter === 'All' || getTeamLevel(team.teamClass) === levelFilter;
    return matchesSearch && matchesSchool && matchesDivision && matchesSeason && matchesLevel;
  });
  ...
  React.useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, schoolFilter, divisionFilter, seasonFilter, levelFilter, view, sortConfig]);
  ...
  return (
    <>
      <div className="flex flex-col gap-8">
        ...
        <Tabs defaultValue="All" value={levelFilter} onValueChange={(value) => setLevelFilter(value as TeamLevel)}>
            <div className="flex items-center justify-between mb-4">
                <TabsList>
                    <TabsTrigger value="All">All Teams</TabsTrigger>
                    <TabsTrigger value="A">A Teams</TabsTrigger>
                    <TabsTrigger value="B">B Teams</TabsTrigger>
                    <TabsTrigger value="C">C Teams</TabsTrigger>
                    <TabsTrigger value="D/Other">D/Other</TabsTrigger>
                </TabsList>
            </div>
